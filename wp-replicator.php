...
*/
/* Copyright:	(C) 2010-2011 Ron Rennick, All rights reserved.
...
function ra_copy_blog_page() {
	...
	$replicate_id = ! empty( $_REQUEST['replicate'] ) ? (int)$_REQUEST['replicate'] : 0;
	$src_blog = false;
	...
	} elseif ( ! empty( $_POST[ 'action' ] ) && $_POST[ 'action' ] == 'copy_blog' ) {
	...
	if ( isset( $msg ) ) {		
	?>
	<div id="message" class="updated fade"><p><strong><?php echo esc_html( $msg ); ?>
	</strong></p></div>
	<?php
	}
...
<?php		if( $src_blog ) {
			?><h4 class="submit">On New Site Creation:&nbsp;
				<input class='button' type='submit' name="default_blog" value='<?php printf( 'Use %s (%s) as the site template', $src_blog->blogname, $src_blog->siteurl ); ?>' />
			</h4><?php
			echo '<h4>' . esc_html( sprintf( __( 'Replicate %s (%s) to' ), $src_blog->blogname, $src_blog->siteurl ) ) . '</h4>';
			echo '<input type="hidden" name="source_blog" value="' . $replicate_id . '" />';
		} else { ?>
			<h4 class="submit">On New Site Creation:&nbsp;
				<input class='button' type='submit' name="default_blog" value='Use the site selected below as the site template' />&nbsp;
				<input class='button' type='submit' name="clear_default" value='Use WordPress default site as the site template' />
			</h4>
			<p><strong>Currently, new sites will be created using</strong> <code>
<?php			$copy_id = get_site_option( 'ra_default_blog', false );
			if( $copy_id ) {
				$template = get_blog_details( $copy_id );
				echo $template->blogname . ' ( ' . $template->siteurl . ' )';
			} else
				echo 'the default WordPress site';
			echo '</code>.</p>';
		} ?>
...
<?php		} ?>
		...
		echo '<h3>' . __( 'Create a sub-site to use the Replicator' ) . '</h3>';
	}
}
...
function ra_create_blog( $email, $domain = '', $title, $username = '', $password = 'N/A', $copy_id = 0 ) {
	...
	wp_mail( get_site_option( 'admin_email' ),  sprintf( __( '[%s] New Site Created' ), $current_site->site_name ), $content_mail, 'From: "Network Admin" <' . get_site_option( 'admin_email' ) . '>' );
...
function ra_copy_blog( $new_id, $copy_id = 0, $user_id = 0 ) {
	...
	$tables = $wpdb->get_results( "SHOW TABLES LIKE '{$blogtables}%'", ARRAY_A );
	if( ! $tables )
		return;
	...
	// check to see if it's the old or new media folder structure
	$copy_media_url = null;
	if ( get_site_option( 'ms_files_rewriting' ) ) {

		$cp_base = ABSPATH . UPLOADBLOGSDIR . '/';
		$copy_path = is_dir( $cp_base . $new_id ) ? '/* ' : ' ';
		$cp_cmd = str_replace( '//', '/', 'cp -r ' . $cp_base . $copy_id . $copy_path . $cp_base . $new_id );
		...
		$copy_dir = $uploads['basedir'];
		$url_path = parse_url( $uploads['baseurl'] );
		$copy_media_url = $url_path['path'];
		...
		$uploads = wp_upload_dir();
		$new_dir = $uploads['basedir'];
		$url_path = parse_url( $uploads['baseurl'] );
		$new_media_url = $url_path['path'];

		restore_current_blog();

		$copy_path = is_dir( $new_dir ) ? '/* ' : ' ';
		$cp_cmd = str_replace( '//', '/', 'cp -r ' . $copy_dir . $copy_path . $new_dir );
	...
	foreach( $data as $k => $v ) {
		...
		foreach( $v as $row ) {
			...
			if ( $is_post ) {
				...
				if ( $copy_media_url )
					$row['post_content'] = str_replace( $copy_media_url, $new_media_url, $row['post_content'] );
	...
	if ( ! empty( $trash ) ) {
		...
		$posts = implode( ',', $trash );
		$wpdb->query( "DELETE FROM {$newtables}postmeta WHERE post_id in ($posts)" );
		$wpdb->query( "DELETE FROM {$newtables}posts WHERE post_parent in ($posts) AND post_type = 'revision'" );
	...
	// update options
	$skip_options = apply_filters( 'replicator_skip_options', array( 
		'admin_email',
		'new_admin_email',
		'blogname',
		'blogdescription',
